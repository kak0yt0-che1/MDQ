...
- [x] SHAP per-card reason codes (for top lead)
...
- [ ] SHAP reason codes exported for **all 11 leads** (only the top-1 lead has reason codes)
- [ ] Calibration of predicted probabilities (raw LightGBM scores may not be well-calibrated)
...
| Priority | Action | Why |
|----------|--------|-----|
| 🔴 **1** | Export SHAP reason codes for all 11 leads → attach "why" column to `hidden_entrepreneurs.csv` | RMs need explanations per card to make outreach calls |
| 🟡 **2** | Uncomment SHAP beeswarm plot in notebook cell 09 | More readable than bar chart for presentations |
| 🟡 **3** | Calibrate probabilities with `CalibratedClassifierCV` | So the 0.30 threshold is interpretable as an actual probability |
